...
- **Owner** - библиотека для управления конфигурацией
...
### Параметры для удаленного запуска (Selenoid)
- `selenoidUser` - логин для подключения к Selenoid (передается из Jenkins credentials)
- `selenoidPassword` - пароль для подключения к Selenoid (передается из Jenkins credentials)
- `selenoidUrl` - URL Selenoid сервера (по умолчанию: selenoid.autotests.cloud/wd/hub)
...
Логин и пароль для Selenoid передаются из Jenkins как системные свойства:
```groovy
systemProperty 'selenoidUser', credentials('selenoid-user')
systemProperty 'selenoidPassword', credentials('selenoid-password')
```
